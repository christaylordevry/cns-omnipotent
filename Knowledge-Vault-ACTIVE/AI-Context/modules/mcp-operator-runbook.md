# Module: MCP operator runbook
#
> **Source-of-truth:** `specs/cns-vault-contract/modules/mcp-operator-runbook.md` (implementation repo).  
> This vault module is the mirror operators actually read during incidents.
#
Load this module when wiring Tier 1 MCP servers, rotating keys after an incident, or proving a live tool-call smoke in Cursor and Claude Code.
#
...
| **Reload required** | Restart Cursor to reload `mcp.json` | Re-run `claude mcp add` (to update), and restart your `claude` session if needed |
...
#
### Claude Code stdio registration pattern (env wired, placeholder-safe)
#
Pattern:
#
```bash
...
3. **Re-run live-call smoke**: perform one remote tool call and capture evidence.
4. **Clean up**: remove any pasted key from chat logs or shared documents when possible, and document the incident at a high level without the secret value.
#
...
- **Host alias vs protocol tool name**
  - If you are in a host UI, you will likely see and invoke `mcp__perplexity__search` (or a similar host-defined alias).
