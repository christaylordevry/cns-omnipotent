...
install_wsl_crontab_lines() {
  ...
  filtered="$(printf '%s\n' "$existing" | grep -v "$CRON_TAG" | grep -v "$WATCHDOG_CRON_TAG" | grep -v "$WATCHDOG_CRON_TAG_AFTERNOON" | grep -v "$WATCHDOG_CRON_TAG_EVENING" | grep -v "$OUTCOME_CHECK_CRON_TAG" | sed '/^[[:space:]]*$/d' || true)"
  {
    printf '%s\n' "$filtered"
    printf '%s\n' "$digest_line"
...
mkdir -p "$(dirname "$OUTCOME_CHECK_LOG_FILE")"
mkdir -p "$(dirname "$JOB_ID_FILE")"
chmod +x "$RUNNER"
chmod +x "$WATCHDOG_RUNNER"
chmod +x "$OUTCOME_CHECK_RUNNER"
...
OUTCOME_CHECK_CRON_EXPR="${DIGEST_OUTCOME_CHECK_CRON:-$DEFAULT_OUTCOME_CHECK_CRON}"
CRON_LINE="${CRON_EXPR} CRON_TZ=Australia/Sydney DIGEST_TRIGGER=cron /bin/bash \"$RUNNER\" >>\"$LOG_FILE\" 2>&1 # $CRON_TAG"
WATCHDOG_CRON_LINE="${WATCHDOG_CRON_EXPR} CRON_TZ=Australia/Sydney DIGEST_TRIGGER=watchdog-0715 /bin/bash \"$WATCHDOG_RUNNER\" >>\"$WATCHDOG_LOG_FILE\" 2>&1 # $WATCHDOG_CRON_TAG"
WATCHDOG_CRON_LINE_AFTERNOON="${WATCHDOG_CRON_EXPR_AFTERNOON} CRON_TZ=Australia/Sydney DIGEST_TRIGGER=watchdog-1300 /bin/bash \"$WATCHDOG_RUNNER\" >>\"$WATCHDOG_LOG_FILE\" 2>&1 # $WATCHDOG_CRON_TAG_AFTERNOON"
WATCHDOG_CRON_LINE_EVENING="${WATCHDOG_CRON_EXPR_EVENING} CRON_TZ=Australia/Sydney DIGEST_TRIGGER=watchdog-1830 /bin/bash \"$WATCHDOG_RUNNER\" >>\"$WATCHDOG_LOG_FILE\" 2>&1 # $WATCHDOG_CRON_TAG_EVENING"
OUTCOME_CHECK_CRON_LINE="${OUTCOME_CHECK_CRON_EXPR} CRON_TZ=Australia/Sydney /bin/bash \"$OUTCOME_CHECK_RUNNER\" >>\"$OUTCOME_CHECK_LOG_FILE\" 2>&1 # $OUTCOME_CHECK_CRON_TAG"
...
install_wsl_crontab_lines "$CRON_LINE" "$WATCHDOG_CRON_LINE" "$WATCHDOG_CRON_LINE_AFTERNOON" "$WATCHDOG_CRON_LINE_EVENING" "$OUTCOME_CHECK_CRON_LINE"
...
echo "Outcome check schedule (CRON_TZ=Australia/Sydney): $OUTCOME_CHECK_CRON_EXPR"
echo "Hermes job schedule: $HERMES_SCHEDULE_DUMMY (dummy; WSL line is the real trigger)"
echo "Log: $LOG_FILE"
echo "Watchdog log: $WATCHDOG_LOG_FILE"
echo "Outcome check log: $OUTCOME_CHECK_LOG_FILE"
...
echo "Override outcome check schedule: DIGEST_OUTCOME_CHECK_CRON"
