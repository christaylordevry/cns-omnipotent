---
name: triage
description: "Hermes CNS Inbox triage for /triage in #hermes. Recursive 00-Inbox/ discovery, deterministic sort, paging (--offset), optional scoped vault_search, non-mutating /approve, governed /execute-approved via vault_move, and auto-synthesis on approval when source_uri qualifies."
version: 1.6.0
author: CNS Operator
license: MIT
metadata:
...
After a successful **`/execute-approved`** move, **auto-synthesis on approval** (Stories 30.1–30.3) may run a shallow research chain and stamp **`verification_status: pending`** on the synthesis output when the moved note has an http(s) **`source_uri`**.
...
- **Execution is narrow and governed** (Story 27.5): `/execute-approved ... --to .../` calls **`vault_move`** once for the approved item; on the synthesis path (Stories 30.1–30.2) it may also call **one** **`vault_update_frontmatter`** on the chain output after a successful **`run-chain`** run.
...
- **Discard vocabulary maps to safe outcomes** (Story 27.6): colloquial “discard”, “delete”, or “archive” means either optional **relocation** with **`/execute-approved`** → **`vault_move`** to an operator-chosen directory (still WriteGate/PAKE-governed when leaving `00-Inbox/`), or **human-only** removal in Obsidian or the filesystem **outside** Hermes—never silent destruction.
...
- Operator posts **`/execute-approved <00-Inbox/path.md> --to <destination_dir>/`** to execute one approved move through **`vault_move`** and, when eligible, **auto-synthesis on approval** into **`03-Resources/`** (Stories 27.5, 30.1–30.3).

## When not to use

- Any vault mutation outside a valid **`/execute-approved`** command (including bulk moves, rename tricks that skip **`vault_move`**, mutators beyond **`vault_move`** plus at most **one** post-chain **`vault_update_frontmatter`**, or direct **`vault_log_action`** apart from what **`vault_move`** emits on success).
...
- **Trigger condition:** After successful **`vault_move`** on **`/execute-approved`**, synthesis runs only when destination frontmatter has a non-empty **`source_uri`** starting with `http` (case-insensitive; `https://` qualifies).
...
- **Governed mutations on `/execute-approved`.** Allowed mutators: **`vault_move`** (always, once per valid command) and **one** **`vault_update_frontmatter`** on the synthesis output path **only** after a successful **`run-chain`** run (Story 30.2). No other mutating Vault IO tools on this skill path.
...
- **Refuse every other mutation.** If asked to mutate outside valid `/execute-approved` (and the allowed post-chain frontmatter stamp), respond with refusal and do nothing.
