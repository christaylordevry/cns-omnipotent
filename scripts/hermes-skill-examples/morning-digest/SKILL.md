...
**Strict collection order:** 1 → 2 → 4 → 5 → 7 → 8 → 9 → 10 → 11 → 12 → 13 → 14 → 15 → 3 → 6 (Source 3 = Perplexity deep signal — **after** Source 15; Source 6 = Vault context — **only after** Sources 9–15 and Source 3 terminals fire).
...
15. Call `terminal(command="bash scripts/session-close/hermes-run-perplexity.sh <shellQuote(top_trend_keyword)>", workdir=resolved_repo_root, timeout=45)` for Perplexity Deep Signal (Source 3 — **after** Source 15; top keyword from Source 1 only).
16. Source 6 (Vault context): build `digest_sources` from adapter stdout; run `terminal(..., timeout=30)` with `node "$PICK_SCRIPT"` where `PICK_SCRIPT` = `scripts/hermes-skill-examples/morning-digest/scripts/pick-signal-notebook.mjs` and `DIGEST_SOURCES_JSON` is shell-quoted (see task-prompt Source 6). On ROUTED, run `query-notebook.mjs` via `node "$QUERY_SCRIPT"` — **never** `bash …pick-signal-notebook.mjs` or `bash …query-notebook.mjs` (ES modules require `node`).
17. **Pre-Discord §9 map:** build `digest_push_payload.signals[]` from **all** adapter stdout per task-prompt §9 signal mapping table — **non-negotiable gate before dedupe** (see task-prompt `Build digest_push_payload.signals from §9 map`).
18. **Pre-Discord:** invoke `terminal(..., timeout=30)` for `dedupe-digest-signals.mjs` with `DIGEST_SIGNALS_JSON=<shellQuote(JSON.stringify(digest_push_payload.signals))>`, capture stdout, parse `deduped_signals = JSON.parse(stdout.trim())`, and assign `digest_push_payload.signals = deduped_signals` when valid (see task-prompt Pre-Discord dedupe section).
19. **Pre-Discord:** invoke `terminal(..., timeout=30)` for `score-digest-signals.mjs`, capture stdout, parse `scored_signals = JSON.parse(stdout.trim())`, and assign `digest_push_payload.signals = scored_signals` when valid (see task-prompt Pre-Discord scoring section).
20. **Pre-Discord:** invoke `terminal(..., timeout=15)` for `write-digest-push-artifact.mjs` with post-dedup, post-scoring `DIGEST_PUSH_JSON` — **before** Discord post (see task-prompt artifact section).
21. Post the final `🌅 **Morning Digest**` contract even when one source fails.
22. After posting, on ROUTED + successful query only, **await** `terminal(..., timeout=15)` for `log-notebook-query.mjs` (telemetry + optional warning; see task-prompt post-post).
23. After posting (all runs), invoke `terminal(..., timeout=45)` for `push-digest-convex.mjs` with post-scoring `DIGEST_PUSH_JSON` — **DO NOT improvise** (no direct Convex HTTP/MCP); fire-and-forget result only (see task-prompt digest Convex push).
24. After digest Convex push (all runs), invoke `terminal(..., timeout=45)` for `push-keyword-candidates.mjs` with the same post-scoring shell-quoted `DIGEST_PUSH_JSON` (fire-and-forget; see task-prompt keyword candidates push).

The final reply must use the task-prompt headings exactly: `🌅 **Morning Digest**`, `**Trending Now**`, `**Headlines**`, `**Deep Signal**`, `**arXiv Preprints**`, `**HackerNews**`, `**X / Twitter**`, `**Bluesky**`, `**YouTube**`, `**TikTok**`, `**Instagram**`, `**Vault context**`, and `**Recommended focus:**`. Never invent trends or headlines when a tool fails.
...
15. Instagram (Source 15): call `terminal(command="bash scripts/session-close/hermes-run-instagram.sh", workdir=resolved_repo_root, timeout=45)`. Let `ig_stdout` = terminal stdout (trimmed); try `ig_json = JSON.parse(ig_stdout)` → read **`ig_json.reels`** only (not `videos[]` or `posts[]`); render Discord bullets or show `- (source unavailable: <short reason>)` under **Instagram**; on failure **continue** to Source 3. Instagram hashtag search is Google-index-backed — sparse results are expected.
16. Deep Signal (Source 3): **after Source 15**, call `terminal(command="bash scripts/session-close/hermes-run-perplexity.sh <shellQuote(top_trend_keyword)>", workdir=resolved_repo_root, timeout=45)` using the top parsed Google Trends keyword from Source 1. If no top trend exists, do not invent a fallback keyword; write `- (source unavailable: no top trend keyword)`. Parse `pplx_json.deepSignal` from stdout or show `- (source unavailable: <short reason>)` under **Deep Signal**; on failure **continue** to Source 6.
17. Vault context (Source 6): record `digest_start_ms` at task start; **after Sources 15 and 3 complete**, call `terminal(..., timeout=30)` with `node "$PICK_SCRIPT"` and shell-quoted `DIGEST_SOURCES_JSON` (trends + headlines + Perplexity Deep Signal text + arxiv + hackernews + github + reddit + rss + producthunt + twitter + bluesky + youtube + tiktok + instagram). On ROUTED, call `query-notebook.mjs` via `node "$QUERY_SCRIPT"` with shell-quoted env values (remaining_s cap per task-prompt). **Never** invoke `.mjs` scripts with `bash`. Partial failure → unavailable bullet only in Vault context.
18. **Pre-Discord §9 map:** build `digest_push_payload.signals[]` from all adapter stdout per task-prompt §9 mapping — **before** dedupe (non-negotiable gate).
19. **Pre-Discord:** invoke `terminal(..., timeout=30)` for `dedupe-digest-signals.mjs`, capture stdout, and replace `digest_push_payload.signals` with parsed deduped output before scoring (see task-prompt Pre-Discord).
20. **Pre-Discord:** invoke `terminal(..., timeout=30)` for `score-digest-signals.mjs`, capture stdout, and replace `digest_push_payload.signals` with parsed scored output before artifact write and Discord post (see task-prompt Pre-Discord).
21. **Pre-Discord:** invoke `terminal(..., timeout=15)` for `write-digest-push-artifact.mjs` with post-dedup, post-scoring `DIGEST_PUSH_JSON` before Discord post.
22. Post the digest contract to `#hermes`.
23. After posting the digest, on ROUTED + successful query only, **await** `terminal(..., timeout=15)` for `log-notebook-query.mjs` (emit `notebook_query_log`; warning on `failed`|`timeout` only; does not alter the digest).
24. After posting (all runs), invoke `terminal(..., timeout=45)` for `push-digest-convex.mjs` with post-scoring shell-quoted `DIGEST_PUSH_JSON` — **DO NOT improvise** (see task-prompt pinned §9 block); emit `digest_convex_push`; failures silent to operator.
25. After digest Convex push (all runs), invoke `terminal(..., timeout=45)` for `push-keyword-candidates.mjs` with the same post-scoring shell-quoted `DIGEST_PUSH_JSON` (emit `keyword_candidates_push`; failures silent to operator).
...
- **Sources 9–15 terminal-fire gate:** `hermes-run-rss.sh`, `hermes-run-producthunt.sh`, `hermes-run-x.sh`, `hermes-run-bluesky.sh`, `hermes-run-youtube.sh`, `hermes-run-tiktok.sh`, and `hermes-run-instagram.sh` terminals **MUST fire** before Source 3 or Source 6 (`node …/pick-signal-notebook.mjs`) or Discord post — skipping any invalidates the run.
- **Source 3 terminal-fire gate:** `hermes-run-perplexity.sh` **MUST fire** after Source 15 and before Source 6 or Discord post — skipping invalidates the run.
