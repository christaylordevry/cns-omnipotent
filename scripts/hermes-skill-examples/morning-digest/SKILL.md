...
- **Digest entity push and snapshot replacement (fire-and-forget):** After the Discord post, run `push-digest-convex.mjs` on every run. It pushes or identity-preserving-rescores `digestRuns` / `digestSignals`, persists assigned IDs, then transactionally replaces entity snapshots. Stdout carries structured push and entity status; stderr carries warnings; never post operator warnings; script always exits 0.
