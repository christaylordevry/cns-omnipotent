...
**Strict collection order:** 1 → 2 → 4 → 5 → 7 → 8 → 9 → 10 → 11 → 12 → 3 → 6 (Source 3 = Perplexity deep signal — **after** Source 12; Source 6 = Vault context — **only after** Sources 9–12 and Source 3 terminals fire).
...
12. Call `terminal(command="bash scripts/session-close/hermes-run-perplexity.sh <shellQuote(top_trend_keyword)>", workdir=resolved_repo_root, timeout=45)` for Perplexity Deep Signal (Source 3 — **after** Source 12; top keyword from Source 1 only).
...
- **Sources 9–12 terminal-fire gate:** `hermes-run-rss.sh`, `hermes-run-producthunt.sh`, `hermes-run-x.sh`, and `hermes-run-bluesky.sh` terminals **MUST fire** before Source 3 or Source 6 (`node …/pick-signal-notebook.mjs`) or Discord post — skipping any invalidates the run.
- **Source 3 terminal-fire gate:** `hermes-run-perplexity.sh` **MUST fire** after Source 12 and before Source 6 or Discord post — skipping invalidates the run.
