...
Bind `#general` only to the capture skill (now a subsection of `hermes-url-ingest-vault`), and keep existing `#hermes` bindings unchanged:
...
discord:
  ...
  channel_prompts:
    ...
    "1484880486785486951": |
      CNS #general auto-capture channel. For any message containing an http:// or https:// URL substring, use hermes-url-ingest-vault (general channel capture-only mode, Story 28.3).
...
`#hermes` keeps governed URL ingest to `03-Resources/` through Vault IO. `#general` uses the capture-only subsection of the same skill for raw Inbox writes, later triaged via Epic 27 workflow.
