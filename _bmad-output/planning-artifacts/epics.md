...
### Epic 34: Vault Health + Cost Optimization

Reduce Hermes auxiliary inference cost, clear critical vault-lint ERRORS from the 2026-05-17 report, and burn down stale `verification_status: pending` debt older than 14 days.

**Stories:** 34-1 (auxiliary Haiku routing), 34-2 (lint critical: duplicate delete + 77-note frontmatter patch), 34-3 (operator `/verify` queue for 22 stale pending)
**Phase:** 6
**Status:** in-progress
...
As the **operator**,
I want **Hermes auxiliary workloads** (compression, session search, skills hub, vision) on **Claude Haiku 4.5**,
So that **background infrastructure calls cost less** without changing the primary Sonnet agent.

**Acceptance criteria:** See `_bmad-output/implementation-artifacts/34-1-auxiliary-model-cost-reduction.md` (four `auxiliary.*.model` keys in `~/.hermes/config.yaml`, gateway restart, smoke test; no vault writes; no test updates).

#### Story 34.2: Vault lint remediation — critical issues

As the **operator**,
I want **Rule 1 duplicate delete and Rule 4 frontmatter ERRORS** from `vault-lint-2026-05-17.md` resolved,
So that **governed notes pass critical lint** before the next ingest cycle.

**Acceptance criteria:** See `_bmad-output/implementation-artifacts/34-2-vault-lint-remediation-critical-issues.md` (delete `e2e-epic30-20260516-cursor-dev.md`, patch 77 notes via `vault_update_frontmatter`, re-lint + `npm test` + `verify.sh`).

#### Story 34.3: Stale pending review via /verify

As the **operator**,
I want to **process 22 stale pending notes** using **`/verify`** (SynthesisNotes) and governed frontmatter updates (other types),
