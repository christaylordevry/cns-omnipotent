...
- [ ] [Review][Patch] Rate-limit exhaustion message does not match story AC3 [`src/agents/anthropic-fetch.ts`] — story requires `"<Adapter> API rate limited after 3 attempts"`; current code throws `Anthropic API rate limited after ${maxAttempts} attempts — ${label}`. This is a behavior mismatch for the public error message contract and also couples message to `adapterLabel` formatting (e.g., `"weapons check"`). Fix either by (a) changing helper to accept `exhaustedMessage` and throw exactly that, or (b) throwing a typed error in helper and letting adapters wrap into the exact message required by the story.
...
- **Shared helper:** `src/agents/anthropic-fetch.ts` exports `fetchWithRetry(url, init, { adapterLabel, maxAttempts? })`. It parses `retry_after` from the JSON body (`error.retry_after`), falls back to the `Retry-After` header, defaults to 5s when neither is present, and clamps the sleep to `[5, 120]` seconds. Retries only on HTTP 429 (up to 3 total attempts); all other non-2xx responses are returned to the adapter untouched. After 3 consecutive 429s, it throws `CnsError("IO_ERROR", "Anthropic API rate limited after 3 attempts — <label>")`.
- **Adapter wiring:** each adapter (`synthesis-adapter-llm.ts`, `hook-adapter-llm.ts`, `boss-adapter-llm.ts`) now calls `fetchWithRetry` with its own `adapterLabel` (`"synthesis"`, `"hook"`, `"weapons check"`). The existing fetch-error catch block also lets `CnsError` propagate unwrapped so the retry-exhaustion message is not re-prefixed with "X LLM fetch failed:".
