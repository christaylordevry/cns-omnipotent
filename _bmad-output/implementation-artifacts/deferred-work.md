...
- **⚠️ WATCH NEXT SESSION-CLOSE — drive-sync PHASE 90s wall-clock timeout (surfaced 2026-07-09)** — the first live `/session-close` after 58-3 wrote the PDF fine but the **fan-out sync phase timed out at 90 s** (targets left `pending`, `failure_class: none`; also an `nlm_auth: timeout` warning that is a FALSE alarm — `server_info` reported `auth_status: configured`, so do NOT `nlm login`). Root cause is transient NotebookLM API latency on the **`nlm source list --drive --json` calls** that run before each sync (3 notebooks): the `nlm source sync` command itself is instant/fire-and-forget (verified: returns `✓ Synced 1 source(s)` immediately), so the sync is NOT the bottleneck — the pre-sync `list` queries are. Manually force-synced all 3 (`nlm source sync … -y`) 2026-07-09 so notebooks carry the current export. **Decision gate:** if the NEXT `/session-close` ALSO times out the sync phase (i.e. systematic, not one flaky night), scope a small hardening — raise the 90 s drive-sync wall-clock budget and/or make the per-notebook `list` calls resilient (retry + shorter per-call timeout, or parallelize the 3). If it comes back clean, close this — NotebookLM's own Drive auto-sync backstops a missed manual sync anyway. See [[project_notebooklm_drive_write_root_cause]].
