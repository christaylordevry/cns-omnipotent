...
## session-close changelog-row duplication bug (2026-07-10)

`/session-close` §8 regeneration bumps the AGENTS version but writes a changelog row that **duplicates the previous row's description** verbatim under the new version number, instead of a descriptive "Section 8: Regenerated by /session-close (…)" entry. Evidenced by three duplicate pairs: `2.1.50/2.1.51`, `2.1.52/2.1.53`, and now `2.1.56/2.1.57` (the 2.1.57 row wrongly repeats the Lane B "Registered HookSetNote…" text). Cosmetic — constitution.test passes, §8 content is correct — but it pollutes the constitution changelog every close and mislabels the session-close version. Fix in `scripts/session-close/lib/apply-section8.mjs` (or wherever the changelog row is composed): emit a self-descriptive §8-regen row (date, new version, e.g. "Section 8: Regenerated by /session-close; N epics done, X/Y in-progress"), not a copy of the prior row.
