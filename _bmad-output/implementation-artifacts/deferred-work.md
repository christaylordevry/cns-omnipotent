...
## QUEUED (fresh session): vault-lint remediation (2026-07-10)

**Ready to execute** — full brief in `HANDOFF-2026-07-10-vault-lint-remediation.md`. First real use of the new gated vault-write path. `bulk_scan.py` (canonical) found **145 governed notes, 116 clean, 13 errors, 31 warnings**. Fix the 13 errors via governed `vault_update_frontmatter` (each write prompts operator; git baseline `227b490` in canonical vault = rollback). Mapping (confirm at session start): `invalid_status: reference` → `reviewed` (7); `invalid_verification_status: unverified` → `pending` (5); missing `created` → copy note's `modified` (5, overlapping). Then re-lint → expect 0 errors. Separate/optional: 14 stale-pending triage, 9 orphans, vault line-ending normalization.
