...
2. **Batch-verify those names** in as few vault IO calls as possible — prefer one `vault_list` or `vault_search` sweep over the candidate set. Fall back to per-target calls only if a single sweep cannot cover the full set. Check existence by FILENAME — an exact-name match or a `vault_list` of the target folder — not by a semantic/content `vault_search` alone, which can return zero hits for a note that exists under an exact filename and make you wrongly drop a real link. If a candidate does not surface in the sweep, confirm with a direct filename lookup before deciding it is missing.
