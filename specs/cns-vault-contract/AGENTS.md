...
> Version: 2.0.0 | Last updated: 2026-05-13
...
- **CNS Phase 6 active. Epic 29 complete.** All stories 29-0 through 29-10 shipped: token audit, USER.md wire, MEMORY.md schema, cold-start verification, vault-lint spec, `/vault-lint` Hermes skill, dedup guard at ingest time, CLAUDE.md shim update + token budget policy, `vault_request_disambiguation` MCP tool, fast-scan index + session-close integration, and Hermes thinking commands (`vault-think`). Epic 29 retrospective is optional.

### Current Priorities

1. **Define and plan Epic 30.** Epic 29 is fully closed; next step is scoping the next epic (Phase 6 continuation or Phase 7 planning).
2. **Run Epic 29 retrospective (optional).** Document lessons from the token-budget and Hermes skills wave before opening Epic 30.
...
- **29-10 (done):** `vault-think` Hermes skill — `/challenge`, `/emerge`, `/ideas` on-demand cognition commands using read-only vault search; zero always-on token overhead; v1.1 stubs (`/trace`, `/connect`, `/ghost`, `/drift`) documented.
- **29-9 (done):** Fast-scan index and session-close integration — `AI-Context/vault-fast-scan-index.md` regenerated each `/session-close`; compact agent catalog under 2,000 tokens; Step 6.6 wired into session-close task prompt.
