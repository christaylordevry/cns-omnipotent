...
> Version: 2.1.4 | Last updated: 2026-05-23
...
- **CNS Phase 7 active. Epic 38 is in progress for cost and provider optimization.** Sprint tracker shows 38-1 and 38-3 done, with 38-2 ready-for-dev as the remaining Epic 38 story.

### Current Priorities

1. **Execute Story 38-2: Kimi K2.6 evaluation for `run-chain`.** Resolve the live model slug, run the controlled smoke, capture PAKE++/latency/quality evidence, and decide adopt/defer/reject.
2. **Keep Epic 38 evidence coherent.** Preserve the provider migration and prompt-cache proof artifacts as the baseline for any cheaper run-chain inference changes.
3. **Maintain AGENTS.md freshness, MEMORY.md, fast-scan index, and NotebookLM export via `/session-close` at each session end.**

### Recent Session Context

- **38-3 prompt cache verification complete:** `prompt_caching.cache_ttl` was aligned to `1h`, gateway restart was verified, and redacted `agent.log` evidence showed post-restart cache usage.
- **38-1 provider migration documented:** Hermes main and auxiliary routing moved to Codex OAuth (`gpt-5.5` / `gpt-5.4-mini`), compression threshold moved to `0.2`, and gateway token-bridge evidence was captured.
- **38-2 Kimi run-chain evaluation ready:** The story defines the remaining Epic 38 work: verify the live Kimi model id, smoke `scripts/run-chain.ts`, compare against Sonnet, and record an adopt/defer/reject recommendation.
