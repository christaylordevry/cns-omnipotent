...
> Version: 2.1.5 | Last updated: 2026-05-23
...
- 43-1-cns-daily-rhythm-auto-blocks-via-session-close: Story 43.1: CNS-Daily-Rhythm AUTO blocks via session-close (done) — Add `references/daily-rhythm-static-rows.md` with non-sprint project rows and epic 39–42 theme fallbacks (AC: projects, roadmap); Implement Step 6.7 in `references/task-p
- 38-3-prompt-cache-hit-rate-verification: Story 38.3: Prompt cache hit rate verification (done) — Snapshot current `prompt_caching` + `model` cache TTL keys (before).; Set `prompt_caching.cache_ttl: 1h`; validate YAML; restart gateway.
- 38-1-provider-migration-documentation: Story 38.1: Provider migration documentation (done) — Back up or diff against `~/.hermes/config.yaml.backup-2026-05-17`.; Set main `model:` block to `openai-codex` / `gpt-5.5` / Codex `base_url` / `prompt_cache_ttl: 1h`.
