...
> Version: 2.0.9 | Last updated: 2026-05-20
...
- **CNS Phase 6 active. Epic 36 in-progress.** Epics 1–35 complete. Epic 36 (Operational Stability + Vault Close-Out): 36-1 done (gateway auto-start), 36-2 done (skill parity pass), 36-3 done (Projects/Areas stale pending + hub indexes). All stories complete; epic ready to close.

### Current Priorities

1. **Close Epic 36 (Operational Stability + Vault Close-Out).** All three stories done. Run epic retro and mark epic-36 done in sprint-status.yaml.
2. **Post-close vault hygiene.** Run `/vault-lint` after Epic 36 retro to confirm Rule 3 stale-pending for 01-Projects/ and 02-Areas/ clusters drops to zero (target of Story 36-3).
3. **Maintain AGENTS.md freshness.** Keep Section 8 and NotebookLM export current via `/session-close` at each work session end.

### Recent Session Context

- **36-3 (done):** 01-Projects/ and 02-Areas/ stale-pending cleared via `/verify`; hub `_README.md` indexes with wikilinks added for WorkflowNotes in both trees; orphan counts reduced.
- **36-2 (done):** Hermes skill repo mirrors synced for session-close, vault-lint, hermes-url-ingest-vault; install scripts added/updated.
- **36-1 (done):** Hermes gateway auto-start via WSL `@reboot` cron; launcher at `scripts/hermes-gateway-start.sh`; MEMORY.md updated to reflect auto-start.
