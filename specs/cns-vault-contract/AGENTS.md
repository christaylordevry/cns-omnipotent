...
> Version: 2.1.47 | Last updated: 2026-06-28
...
- Epic 73: in-progress — Story 73-8 (entity-intelligence health, live prod) ready-for-dev
- Epic 76: in-progress (no stories tracked yet)
- Epic 77: in-progress — Story 77-3 (Convex webhook push for high-signal events) ready-for-dev; Story 77-4 (awareness-sync Hermes skill) in review
- Epic 78: in-progress — Story 78-2 (per-skill Hermes model routing activation) in review
- Epic 79: in-progress — Story 79-6 (chunked brain index, chunk-aware recall) in review; Story 79-7 (recall calibration soften quality weighting) in review
- Epic 82: in-progress — Story 82-1 (SPIKE-OMNI-001: SvelteKit → `:9119/api/ws` auth ticket) ready-for-dev

### Current Priorities

1. Fix pre-existing vitest failures (brain index tests + cns-brain-recall shadow→live promotion) so session-close runs clean.
2. Pick up Story 73-8: entity-intelligence health endpoint for live prod.
3. Advance Story 82-1 (SPIKE-OMNI-001 ws-proxy) to dev and close reviews on 78-2, 79-6, 79-7.

### Recent Session Context

- Session-close unblocked: fixed `defaultSessionCloseEnvPath` using `homedir()` instead of `env.HOME` — `drive_doc_id` was leaking into fanout target assertions (2026-06-28).
- Story 82-1: SPIKE-OMNI-001 findings artifact created — SvelteKit → `:9119/api/ws` auth ticket, ready-for-dev.
- Story 79-7: recall calibration soften quality weighting — tunable α + threshold retune, in review (baseline 4b1eba7).
...
Protect the main session transcript from large or orthogonal work by delegating to a subagent (Claude Code **Agent** tool, Cursor **Task** tool) instead of loading everything into the parent context.

**Spawn a subagent when either applies:**

- The task will add roughly **10k tokens or more** of new context (wide codebase exploration, many file reads, long MCP or web dumps, full test logs) that the parent does not need verbatim.
- The task does **not** require the full session history (standalone research, isolated implementation on a fixed path, CI investigation, adversarial review of a named diff).

**Keep work in the main session when:**

- The user is iterating on a small, coherent change set and needs prior turns in context.
- You must coordinate tightly with vault mutations, WriteGate, or constitution edits the parent is already holding.
- The deliverable is a short answer from one or two small reads.

**How to delegate:** Give the subagent a self-contained prompt (goal, paths, constraints, expected output shape). Return only a summary, file list, and blockers to the parent. Do not paste full subagent transcripts unless the user asks.

| Surface | Typical subagent types |
|---------|------------------------|
| Claude Code | Agent tool `subagent_type` (e.g. Explore, general-purpose, shell) |
| Cursor | Task tool (`explore`, `generalPurpose`, `shell`, `best-of-n-runner`) |

### Parallel Sessions (Git Worktrees)

When two or more AI sessions may edit the same implementation repo concurrently (for example Omnipotent.md), isolate each session in its own git worktree so branches and dirty files do not collide.

**Prefer the harness over raw git:** On Claude Code, use **`EnterWorktree`** to create or switch into an isolated checkout and **`ExitWorktree`** when finished. Do not run `git worktree add` manually when these tools are available; the harness owns placement, branch naming, and cleanup.

**Typical workflow:**

1. Before parallel or background implementation, call **`EnterWorktree`** (or start the session with `--worktree`).
2. Confirm isolation: `git rev-parse --git-dir` and `git rev-parse --git-common-dir` should resolve to **different** paths (linked worktree, not the main checkout). If they match, you are still on the main tree.
3. Run installs, tests, edits, and commits from the worktree root (`git rev-parse --show-toplevel`).
4. Push or open a PR from the worktree branch as usual.
5. Call **`ExitWorktree`** when the session ends or you must return to the main checkout.

**This repo:** Claude-managed worktrees live under **`.claude/worktrees/`** (gitignored). List with `git worktree list`. Remove orphans only after confirming no active session: `git worktree remove <path> --force`.

**Claude Code settings (operator):** `worktree.baseRef: "head"` keeps unpushed commits in new worktrees; default `"fresh"` branches from `origin/<default>`. `worktree.bgIsolation: "worktree"` blocks main-checkout edits until `EnterWorktree` when background agents run; set `"none"` only when worktrees are impractical for the repo.

**Cursor:** Background or best-of-N runs that need isolation should use the platform worktree path (for example `best-of-n-runner`), not concurrent edits on the main checkout.
...
| 2026-06-04 | 2.1.32 | Added Behavioral Integrity subsection to Section 1: anti-confabulation guardrail for Nexus/Discord (no fabricated failure reasons, no user-facing command instructions, no claimed hidden work). |
| 2026-06-04 | 2.1.31 | Added Behavioral Integrity subsection to Section 1: anti-confabulation guardrail for Nexus/Discord (no fabricated failure reasons, no user-facing command instructions, no claimed hidden work). |
| 2026-06-04 | 2.1.30 | Added Behavioral Integrity subsection to Section 1: anti-confabulation guardrail for Nexus/Discord (no fabricated failure reasons, no user-facing command instructions, no claimed hidden work). |
| 2026-06-04 | 2.1.29 | Added Behavioral Integrity subsection to Section 1: anti-confabulation guardrail for Nexus/Discord (no fabricated failure reasons, no user-facing command instructions, no claimed hidden work). |
| 2026-06-04 | 2.1.28 | Added Behavioral Integrity subsection to Section 1: anti-confabulation guardrail for Nexus/Discord (no fabricated failure reasons, no user-facing command instructions, no claimed hidden work). |
...
| 2026-05-30 | 2.1.17 | Story 28.1: Section 8 regenerated by Hermes `/session-close`; current focus now derives from sprint-status.yaml and recent story artifacts. |
| 2026-05-28 | 2.1.15 | Story 28.1: Section 8 regenerated by Hermes `/session-close`; current focus now derives from sprint-status.yaml and recent story artifacts. |
| 2026-05-28 | 2.1.14 | Story 28.1: Section 8 regenerated by Hermes `/session-close`; current focus now derives from sprint-status.yaml and recent story artifacts. |
| 2026-05-28 | 2.1.13 | Story 28.1: Section 8 regenerated by Hermes `/session-close`; current focus now derives from sprint-status.yaml and recent story artifacts. |
| 2026-05-28 | 2.1.12 | Story 28.1: Section 8 regenerated by Hermes `/session-close`; current focus now derives from sprint-status.yaml and recent story artifacts. |
| 2026-05-26 | 2.1.11 | Story 28.1: Section 8 regenerated by Hermes `/session-close`; current focus now derives from sprint-status.yaml and recent story artifacts. |
| 2026-05-26 | 2.1.10 | Story 28.1: Section 8 regenerated by Hermes `/session-close`; current focus now derives from sprint-status.yaml and recent story artifacts. |
| 2026-05-26 | 2.1.9 | Story 28.1: Section 8 regenerated by Hermes `/session-close`; current focus now derives from sprint-status.yaml and recent story artifacts. |
| 2026-05-24 | 2.1.8 | Story 28.1: Section 8 regenerated by Hermes `/session-close`; current focus now derives from sprint-status.yaml and recent story artifacts. |
| 2026-05-24 | 2.1.7 | Story 28.1: Section 8 regenerated by Hermes `/session-close`; current focus now derives from sprint-status.yaml and recent story artifacts. |
| 2026-05-24 | 2.1.6 | Story 28.1: Section 8 regenerated by Hermes `/session-close`; current focus now derives from sprint-status.yaml and recent story artifacts. |
| 2026-05-23 | 2.1.5 | Story 28.1: Section 8 regenerated by Hermes `/session-close`; current focus now derives from sprint-status.yaml and recent story artifacts. |
| 2026-05-23 | 2.1.4 | Section 9: subagent context-isolation rule (10k-token / no-session-history heuristic) and parallel-session git worktree workflow (`EnterWorktree` / `ExitWorktree`, `.claude/worktrees/`). |
| 2026-05-23 | 2.1.3 | Story 28.1: Section 8 regenerated by Hermes `/session-close`; current focus now reflects Epic 38 in progress, with 38-2 Kimi run-chain evaluation next. |
| 2026-05-22 | 2.1.2 | Story 28.1: Section 8 regenerated by Hermes `/session-close`; Phase 6 remains complete and current focus is next-phase scoping plus maintenance freshness. |
| 2026-05-21 | 2.1.1 | Story 28.1: Section 8 regenerated by Hermes `/session-close`; vault fully clean (115 notes, 0 lint issues); hub PAKE frontmatter fixed. |
| 2026-05-21 | 2.1.0 | Story 28.1: Section 8 regenerated by Hermes `/session-close`; all epics 1-37 complete; Phase 6 close-out done; vault baseline 23 R2 orphans accepted. |
| 2026-05-20 | 2.0.9 | Story 28.1: Section 8 regenerated by Hermes `/session-close`; Epic 36 all stories done; current focus is epic retro and vault close-out. |
| 2026-05-18 | 2.0.8 | Story 28.1: Section 8 regenerated by Hermes `/session-close`; current focus now derives from sprint-status.yaml and recent story artifacts. |
| 2026-05-18 | 2.0.7 | Story 28.1: Section 8 regenerated by Hermes `/session-close`; current focus now derives from sprint-status.yaml and recent story artifacts. |
| 2026-05-18 | 2.0.6 | Story 28.1: Section 8 regenerated by Hermes `/session-close`; Epic 34 (vault lint remediation) active; 33-3 still in review. |
| 2026-05-17 | 2.0.5 | Story 28.1: Section 8 regenerated by Hermes `/session-close`; 33-3 in review (Operator Guide Phase 6 complete), all Phase 6 commands covered, Epic 34 planning queued. |
