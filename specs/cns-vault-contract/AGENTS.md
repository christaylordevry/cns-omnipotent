...
> Version: 1.9.4 | Last updated: 2026-05-07  
...
- **CNS Phase 5: IN PROGRESS.** Hermes operator closure and session automation. Epics 26 and 27 are **done** (Hermes WSL2, Vault IO write path, Discord surface, daily digest, skill capture, Inbox triage, approve/execute-approved, non-destructive guarantees). Epic 28 (Hermes operator closure and NotebookLM freshness) is **done** — stories 28-1 and 28-2 shipped, retrospective closed.

### Current Priorities

1. **Begin Epic 29 planning.** Hermes `/session-close` is live and NotebookLM MCP is registered. Identify and define the next epic or Phase 5 wrap-up retrospective.
...
- **28-2 (done):** `Register NotebookLM MCP server in Hermes config (uvx)` — `mcp_servers.notebooklm` wired via `uvx --from notebooklm-mcp-cli notebooklm-mcp`; `source_add` and `notebook_query` callable end-to-end from Hermes; session-close skill patched to call `mcp__notebooklm__source_add` directly.
- **28-1 (done):** `Automate AGENTS.md Section 8 via Hermes session-close` — skill package at `~/.hermes/skills/cns/session-close/`; trigger wired to `#hermes`; filesystem AGENTS sync (both copies), export, and NotebookLM fan-out covered. Live verified.
